...
1. 頁面先從報表站的非個資狀態查詢顯示「訂位資料更新至哪一天」，再由管理員選日期範圍同步 booking record；大量資料由瀏覽器循序呼叫、後端每批最多處理 500 筆並回傳累計進度，避免 Netlify function 30 秒逾時。之後上傳當期 discount offer／套餐。只有報表缺漏時才用 UTF-8 CSV／JSON 手動補資料。
