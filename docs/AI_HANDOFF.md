...
同一區也有「活動參與（可包含或排除）」：可選「參加過任一活動」或「尚未參加任何
活動」，資料口徑是 `activity_plays.line_user_id` 的實際遊玩紀錄。只開活動頁但沒有開始
玩仍算未參加。活動參與與加入時間、生命週期、獎品等其他已選條件使用 **AND**；例如
「7 天內加入」加「尚未參加任何活動」只會選中兩者同時成立的人。頁面另有「一鍵套用
這個範例」，會先清空其他條件再套用此組合，避免殘留條件造成名單過少。預覽與正式
發送同樣共用 `src/core/broadcastAudience.js` 的 `EXISTS`／`NOT EXISTS` SQL；正式環境
已有 `activity_plays(line_user_id)` 索引，不需額外 migration。
