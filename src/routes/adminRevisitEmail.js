...
function selectEligibleCandidates({
  bookings = [], offers = [], sentRows = [], unsubscribedEmails = [], settings,
  ...
  const excluded = {
    not_due: 0, unsubscribed: 0, already_sent: 0,
...
function registerAdminRevisitEmailRoutes(app, deps) {
  ...
  app.get('/admin/revisit-email/api/data', requireAdmin, async (_req, res) => {
    try {
      const [settings, stats, imports, campaigns] = await Promise.all([
  ...
  app.post('/admin/revisit-email/api/generate', requireOwner, async (req, res) => {
    ...
    try {
      ...
      const [unsubRs, sentRs, offersRs] = await Promise.all([
        emails.length
    ...
    try {
      const current = await query(
        `SELECT r.*, c.status AS campaign_status FROM revisit_email_recipients r
         JOIN revisit_email_campaigns c ON c.id = r.campaign_id WHERE r.id = $1`, [Number(req.params.id)]
      );
      if (!current.rowCount) return jsonError(res, 404, 'recipient_not_found');
      const row = current.rows[0];
      if (row.campaign_status !== 'draft' || row.status !== 'pending') return jsonError(res, 409, 'not_editable', '只有尚未寄送的草稿可以修改。');
      const origin = publicBaseUrl(req, configuredPublicBaseUrl, resolvePublicSiteOrigin);
      if (!isHttpUrl(origin) || !origin.startsWith('https://')) {
  ...
  app.post('/admin/revisit-email/api/recipients/:id(\\d+)/test', requireOwner, async (req, res) => {
    ...
    try {
      const rs = await query(`SELECT * FROM revisit_email_recipients WHERE id = $1`, [Number(req.params.id)]);
      if (!rs.rowCount) return jsonError(res, 404, 'recipient_not_found');
      const row = rs.rows[0];
      const testHtml = renderRevisitHtml({
        subject: `[測試] ${row.subject}`, preheader: row.preheader,
        restaurantName: row.restaurant_name, recipientName: row.recipient_name,
        bodyText: row.body_copy, ctaUrl: row.cta_url,
        ctaLabel: row.offer_snapshot && row.offer_snapshot.offerType === 'set_menu' ? '查看套餐並訂位' : row.offer_snapshot ? '查看優惠並訂位' : '查看餐廳並訂位',
        offer: row.offer_snapshot
      });
      const sent = await smtpEmailProvider.sendEmail({
        to: testEmail, subject: `[測試] ${row.subject}`, html: testHtml,
        text: `[測試信，不會影響正式寄送]\n\n${row.body_copy}\n\n${row.cta_url}`
      });
      if (!sent.ok) return jsonError(res, 502, 'smtp_send_failed', sent.error);
  ...
  app.post('/admin/revisit-email/api/campaigns/:id(\\d+)/send', requireOwner, async (req, res) => {
    ...
    try {
      await client.query('BEGIN');
      const campaignRs = await client.query(
        `SELECT status FROM revisit_email_campaigns WHERE id=$1 FOR UPDATE`, [campaignId]
      );
    ...
    return res.json({
      ok: true, claimed: claimed.length, sent: sentCount, needs_review: reviewCount,
  ...
  app.post('/admin/revisit-email/api/campaigns/:id(\\d+)/retry-review', requireOwner, async (req, res) => {
    try {
      const reset = await query(
        `UPDATE revisit_email_recipients SET status='pending', failure_detail=NULL, updated_at=NOW()
          WHERE campaign_id=$1 AND status='needs_review' RETURNING id`, [Number(req.params.id)]
      );
      return res.json({ ok: true, reset: reset.rowCount });
    } catch (err) {
      return schemaError(res, err);
    }
  });
  ...
  app.get('/email/revisit/open/:token([a-f0-9]{48}).gif', async (req, res) => {
    try {
      await query(`UPDATE revisit_email_recipients SET opened_at=COALESCE(opened_at,NOW()), updated_at=NOW()
  ...
  app.get('/email/revisit/click/:token([a-f0-9]{48})', async (req, res) => {
    try {
      ...
      if (!found.rowCount || !isHttpUrl(found.rows[0].cta_url)) return res.redirect('https://tw.openrice.com/');
  ...
  app.get('/email/revisit/unsubscribe/:token([a-f0-9]{48})', async (req, res) => {
    try {
      ...
      if (!found.rowCount) return res.status(404).type('text/plain').send('這個退訂連結無效或已過期。');
